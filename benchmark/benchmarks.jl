...
# Import internal functions used in component benchmarks
using AsteroidThermoPhysicalModels: update_flux_all!, update_flux_sun!, update_flux_scat_single!, update_flux_rad_single!, update_temperature!, energy_in, energy_out
...
"""
Setup Ryugu TPM problem for benchmarking. Returns `(problem, ephem)`.
"""
function setup_ryugu_tpm(n_cycles::Int=20)
    ...
    return problem, ephem
end


"""
Setup Didymos-Dimorphos binary TPM problem for benchmarking. Returns `(problem, ephem)`.
"""
...
SUITE["ryugu"]["full_simulation_20_rotations"] = @benchmarkable begin
    ...
    problem, ephem = setup_ryugu_tpm(20)
    times_to_save = Float64[]
    face_ID = Int[]
end teardown = begin end
...
SUITE["ryugu"]["single_rotation"] = @benchmarkable begin
    ...
    problem, ephem = setup_ryugu_tpm(1)
    times_to_save = Float64[]
    face_ID = Int[]
end
...
SUITE["didymos"]["full_simulation_20_rotations"] = @benchmarkable begin
    ...
    problem, ephem = setup_didymos_tpm(20)
    times_to_save = Float64[]
    face_ID_pri = Int[]
    face_ID_sec = Int[]
end teardown = begin end
...
SUITE["didymos"]["single_rotation"] = @benchmarkable begin
    ...
    problem, ephem = setup_didymos_tpm(1)
    times_to_save = Float64[]
    face_ID_pri = Int[]
    face_ID_sec = Int[]
end
...
SUITE["components"]["ryugu_flux_calculation"] = @benchmarkable begin
    ...
    AsteroidThermoPhysicalModels.init_temperature!(stpm, 200.0)
end
...
SUITE["components"]["didymos_flux_calculation"] = @benchmarkable begin
    ...
    problem, ephem = setup_didymos_tpm(1)
    stpm1 = AsteroidThermoPhysicalModels._build_single_tpm(problem.primary,   CrankNicolson())
    stpm2 = AsteroidThermoPhysicalModels._build_single_tpm(problem.secondary, CrankNicolson())
    btpm = AsteroidThermoPhysicalModels.BinaryAsteroidThermoPhysicalModel(
        stpm1, stpm2,
